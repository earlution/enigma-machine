        ...
        if reverse:
            ...
            for rotr in self.rotors[1:]:
                ...
                if rotr.position != 1:
                    ...
                    print(f' therefore adjusted output to rotor {rotr.rotor_number} ({rotr.__str__()}) is: {letter}')  # for testing...

                    # encode
                    letter = rotr.encode(letter)
                    print(f'Reflected rotor {rotr.rotor_number} ({rotr.__str__()}) encoded: {letter}')  # for testing...

                    # adjust
                    letter = self.rotate_letter(letter, - self.rotors[curr_rotor_index].position + 1)
                    print(f' therefore adjusted output from rotor {rotr.rotor_number} ({rotr.__str__()}) is: {letter}')  # for testing...
